...
sed -i "s/OpenWrt /OpenWrt_x64_旁路由版 by ${Author} build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" "${ZZZ_PATH}"
...
sed -i "/exit 0/i\sed -i 's/\/etc\/coremark.sh/host/g' /etc/crontabs/root" "${FIN_PATH}"
