...
#添加kenzok8插件包
git clone https://github.com/waynesg/OpenWrt-Software package/x64_aptcmk-Software

cat >$NETIP <<-EOF
uci set network.lan.ipaddr='192.168.1.10'                    # IPv4 地址(openwrt后台地址)
uci set network.lan.netmask='255.255.255.0'                  # IPv4 子网掩码
uci set network.lan.gateway='192.168.1.10'                   # IPv4 网关
uci set network.lan.broadcast='192.168.1.255'                # IPv4 广播
uci set network.lan.dns='192.168.1.68'                       # DNS(多个DNS要用空格分开)
...
sed -i "s/OpenWrt /OpenWrt_x64_aptcmk定制版 by GXNAS build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" "$ZZZ_PATH"
