...
uci set network.lan.gateway='192.168.1.250'                   # IPv4 网关
...
sed -i "s/OpenWrt /OpenWrt_x64_京东脚本版 by GXNAS build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ_PATH
...
mv openwrt-x86-64-generic-squashfs-combined.img.gz OpenWrt-x64_jd.img.gz
EOF
